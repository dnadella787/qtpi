...
fn main() {
    ...
    let providers = [
        ("git-minimal.json", "git-minimal.qtpi-provider"),
        ("kubectl-minimal.json", "kubectl-minimal.qtpi-provider"),
    ];
    ...
    for (source_name, output_name) in providers {
        ...
        qtpi_build::compile_json_file_to_file(&source_path, &output_path).unwrap_or_else(|error| {
            panic!("failed to compile built-in provider fixture {source_name}: {error}")
        });
    }
}
